...
def get_args():
    ...
    parser.add_argument('--batch_size', type=int, default=32, help='Batch size')  #256-4
    ...
    parser.add_argument('--patch_size', type=int, default=16, help='Patch size') #2 - 8
    parser.add_argument('--mask_rate', type=float, default=0.2, help='Mask rate')
    parser.add_argument('--save_every', type=int, default=50, help='Save every n epoch')
...
def train(
    ...
    model.train()
    training_loss = []
    for i, ((mask_data), (train_data)) in enumerate(zip(train_loader_masked, train_loader_origin)):
...
def test(
    ...
    compare = []
    with torch.no_grad():
        for i, ((mask_data), (test_data)) in enumerate(zip(test_loader_masked, test_loader_origin)):
            mask_data = mask_data.to(device)
                ...
                for j in range(show_img_count):
                    compare_img = torch.cat(
                        [
                            test_data[j:j+1],
                            mask_data[j:j+1], 
                            decoded.view(batch_size, 1, 256, 256)[j:j+1]
                        ]
                    )
...
def pretrain():
    ...
    for epoch in range(args.epoch):
        ...
        if (epoch+1) % args.save_every == 0:
            torch.save(model.state_dict(), f'./ckpt/maskencoder_32/{epoch+1}epoch.pth')
