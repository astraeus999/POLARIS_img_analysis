...
class ResidualBottleneck(nn.Module):
    ...
    def __init__(self, in_channels, out_channels):
        ...
        self.branch1 = nn.Sequential(
            ...
            ConvBnReLu(in_channels // 2, out_channels // 2, 1, 1, 0)
        )
        self.branch2 = nn.Sequential(
            ConvBnReLu(in_channels // 2, in_channels // 2, 1, 1, 0),
            nn.Conv2d(in_channels // 2, in_channels // 2, 3, 1, 1, groups=in_channels // 2),
            nn.BatchNorm2d(in_channels // 2),
            ConvBnReLu(in_channels // 2, out_channels // 2, 1, 1, 0)
        )
...
class ResidualDownsample(nn.Module):
    ...
    def __init__(self, in_channels, out_channels):
        ...
        self.branch1 = nn.Sequential(
            ...
            ConvBnReLu(in_channels, out_channels // 2, 1, 1, 0)
        )
        self.branch2 = nn.Sequential(
            ConvBnReLu(in_channels, out_channels // 2, 1, 1, 0),
            nn.Conv2d(out_channels // 2, out_channels // 2, 3, 2, 1, groups=out_channels // 2),
            nn.BatchNorm2d(out_channels // 2),
            ConvBnReLu(out_channels // 2, out_channels // 2, 1, 1, 0)
        )
        self.channel_shuffle = ChannelShuffle(2)
...
class Unet(nn.Module):
    '''
    simple Unet with a vector bottleneck in the middle
    '''
    def __init__(
        self,
        timesteps,
        ...
        # 计算每层的 in/out channels
        channels = self._cal_channels(base_dim, dim_mults)

        # 初始卷积：concat 原图和条件图
        self.cond_ch = in_channels
        self.init_conv = ConvBnReLu(in_channels + self.cond_ch, base_dim, 3, 1, 1)
        ...
        self.final_conv = nn.Sequential(
            nn.Conv2d(in_channels=channels[0][0] // 2, out_channels=out_channels, kernel_size=1)
        )

    def forward(self, x, x_cond, t=None):
        # init
        x = self.init_conv(torch.cat([x, x_cond], dim=1))
        if t is not None:
        ...
        x = self.mid_block(x)
        # —— 1D 向量瓶颈 ——
        b, c, h_mid, w_mid = x.shape
        # 全局平均池化得到 (b, c)
        v = x.mean(dim=(2, 3))
        # 再从向量重建回 (b, c, h_mid, w_mid)
        x = v.view(b, c, 1, 1)
        x = F.interpolate(x, size=(h_mid, w_mid), mode='bilinear', align_corners=False)

        # 解码
        shortcuts.reverse()
        for dec, sc in zip(self.decoder_blocks, shortcuts):
            x = dec(x, sc, t)
...
#
#
#     #####  change it to ReLU. ConvBnSiLu
#     #### Change it to a conditional model, timestep is conditional, but need to conditional on the original image
#     ### this is a soft version of adding conditional information, will try hard version later
# class ResidualBottleneck(nn.Module):
#     '''
...
#                                     nn.BatchNorm2d(in_channels//2),
#                                     ConvBnReLu(in_channels//2,out_channels//2,1,1,0))
#         self.branch2=nn.Sequential(ConvBnReLu(in_channels//2,in_channels//2,1,1,0),
#                                     nn.Conv2d(in_channels//2,in_channels//2,3,1,1,groups=in_channels//2),
#                                     nn.BatchNorm2d(in_channels//2),
#                                     ConvBnReLu(in_channels//2,out_channels//2,1,1,0))
#         self.channel_shuffle=ChannelShuffle(2)
...
#                                     nn.BatchNorm2d(in_channels),
#                                     ConvBnReLu(in_channels,out_channels//2,1,1,0))
#         self.branch2=nn.Sequential(ConvBnReLu(in_channels,out_channels//2,1,1,0),
#                                     nn.Conv2d(out_channels//2,out_channels//2,3,2,1,groups=out_channels//2),
#                                     nn.BatchNorm2d(out_channels//2),
#                                     ConvBnReLu(out_channels//2,out_channels//2,1,1,0))
#         self.channel_shuffle=ChannelShuffle(2)
...
#     '''
#
#     #For binary classification tasks, out_channels=1 is often used to output a single probability map.
#     def __init__(self,timesteps,time_embedding_dim,in_channels=1,out_channels=1,base_dim=128,dim_mults=[2,4,8,16]):
#         super().__init__()
#         assert isinstance(dim_mults,(list,tuple))
#         assert base_dim%2==0
#
...
#
#     def forward(self,x, x_cond,t=None):
#         #cond is the original image add a random noise
#         # x=self.init_conv(x)
#         x = self.init_conv(torch.cat([x, x_cond], dim=1))
...
#     t=torch.randint(0,500,(3,))  # t=500
#     model=Unet(500,128)  #t = 500
#     y=model(x,t)
#     print(y.shape)
#
#
